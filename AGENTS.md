...
- Current admin browser routes include `/admin/`, `/admin/accounts`, and `/admin/sandbox_service`; keep admin routes and role-gated APIs explicit when changing shared `ui/` code. Admin Diagnostics exposes current-process expvar, redacted runtime configuration, pprof discovery, bounded recent failures, and on-demand per-request correlation of a bounded lifecycle-event tail with the current GitHub Job result. Keep diagnostic lookups admin-only, bounded, credential-free, and useful when remote lookups fail.
