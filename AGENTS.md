...
- Current admin browser routes include `/admin/`, `/admin/accounts`, `/admin/runner_requests`, `/admin/runner_requests/{id}`, `/admin/sandbox_service`, and `/admin/diagnostics`; keep admin routes and role-gated APIs explicit when changing shared `ui/` code. Treat a Runner request as its own resource: the collection accepts exact Runner Name or internal Request ID lookup, while the canonical detail path uses the internal ID and aggregates persisted state, a bounded lifecycle-event tail, logs, findings, and the current GitHub Job result. Active request details refresh automatically, also support explicit refresh, and must ignore stale responses after navigation. Reserve Admin Diagnostics for current-process expvar, redacted runtime configuration, and pprof discovery. Keep diagnostic lookups admin-only, bounded, credential-free, useful when remote lookups fail, and keep legacy diagnostic deep links compatible.
