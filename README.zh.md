...
手动触发。同版本只有在 GitHub 提供的大小和 digest 仍与 pin 一致时才是 no-op，
且不会下载归档；元数据漂移会失败关闭。发现更高版本后，工作流只接受规范的
Linux x64 资产，对下载归档重新计算 SHA-256 与字节数，并与 GitHub release 元数据
核对；超过 16 个分块、256 MiB 上限的归档会被拒绝。随后工作流再更新公共 pin 和
