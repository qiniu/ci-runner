...
`/runner_requests/{id}/diagnostics` is admin-only, runs on demand, and uses the canonical internal Request ID. It combines the stored request state, the newest 200 `control_log` lifecycle events, and GitHub Job evidence. After a Sandbox becomes ready and before its Runner starts, runnerd records the selected Sandbox region and resolved physical template ID, then runs a best-effort five-second probe that may add the image version from `IMAGE_VERSION` or `ImageVersion` and the preinstalled GitHub Actions Runner version from `/opt/actions-runner/bin/Runner.Listener --version`. The probe decodes only those two allowlisted values, caps each at 256 bytes, and never persists or logs the complete environment, proxy values, tokens, keys, or cache credentials. Runner registration leaves GitHub's official self-update enabled. Immediately before a job starts, the fixed runnerd hook validates the writable Runner copy's version and emits one bounded internal control marker before `RUNNERD_JOB_STARTED`. The Sandbox service accepts only the first valid pre-job marker, freezes that value in memory, ignores later Workflow output for version evidence, and stores it as `effective_runner_version` only through the matching Sandbox/PID exit callback. The frozen value survives provider cleanup even when no command result is returned; a runnerd restart after capture may instead leave this best-effort field empty because recovery never trusts post-start Workflow output. Probe or capture failure does not block Runner startup, completion, or cleanup; known region and template identity still persist, while unavailable values remain empty. A fresh retry or mismatched-Job requeue clears the prior environment snapshot. Region-specific physical template IDs are stored only on Runner Requests and never written back to managed Runner Specs. Existing SQLite tables gain nullable columns additively, so historical rows remain valid.
