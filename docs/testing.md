...
Sandbox service API URL and API Key are not configured in `runnerd.yaml`. After signing in, `/repositories` shows the effective Sandbox source for the selected account or organization but never embeds the credential editor. When configuration is missing, a manageable scope links to `/account/preferences` or `/organizations/{login}/preferences`; Settings remains the single write surface. For organization installations, visibility can come from repository-only access, while Sandbox management requires an active GitHub organization membership with `role == admin` (an organization owner). Settings lists only the signed-in account and owner-manageable organizations. Ordinary members, outside collaborators, and empty/unknown roles receive only redacted readiness (`manageable: false` plus the effective source) and cannot read API URL, secret state, Cache S3 fields, inheritance-account metadata, templates, runner-created instances, or scoped custom Runner Specs. Failed membership lookups reject the request without returning organization configuration. A local runnerd admin role does not bypass this GitHub organization-owner check. The disabled-by-default platform fallback remains admin-managed at `/admin/sandbox_service`. Its audience is either `all` or `selected`; selected entries match the repository owner's stable GitHub account ID and type. The API Key is encrypted with `auth.encryption_key`. Resolution order is a saved runner-request snapshot, installation custom/inherited settings, an eligible personal account, the enabled and audience-eligible admin default, then a not-configured error.
...
`/runner_requests/{id}/diagnostics` is admin-only, runs on demand, and uses the canonical internal Request ID. It combines the stored request state, the newest 200 `control_log` lifecycle events, and GitHub Job evidence. After a Sandbox becomes ready and before its Runner starts, runnerd records the selected Sandbox region and resolved physical template ID, then runs a best-effort five-second probe that may add the image version from `IMAGE_VERSION` or `ImageVersion` and the preinstalled GitHub Actions Runner version from `/opt/actions-runner/bin/Runner.Listener --version`. The probe decodes only those two allowlisted values, caps each at 256 bytes, and never persists or logs the complete environment, proxy values, tokens, keys, or cache credentials. Runner registration leaves GitHub's official self-update enabled. Immediately before a job starts, the fixed runnerd hook validates the writable Runner copy's version and sends one bounded marker plus `RUNNERD_JOB_STARTED` through a permission-restricted one-shot FIFO to the outer startup process; official hook stdout remains in the GitHub Job log and is not observed by the Sandbox command callback. The Sandbox service accepts only the first valid pre-job marker forwarded by that outer process, freezes that value in memory, ignores later Workflow output for version evidence, and stores it as `effective_runner_version` only through the matching Sandbox/PID exit callback. The hook removes the FIFO before Workflow steps begin, and the outer process removes or stops any unused channel during cleanup. The frozen value survives provider cleanup even when no command result is returned; a runnerd restart after capture may instead leave this best-effort field empty because recovery never trusts post-start Workflow output. Probe or capture failure does not block Runner startup, completion, or cleanup; known region and template identity still persist, while unavailable values remain empty. A fresh retry or mismatched-Job requeue clears the prior environment snapshot. Region-specific physical template IDs are stored only on Runner Requests and never written back to managed Runner Specs. Existing SQLite tables gain nullable columns additively, so historical rows remain valid.

Custom Runner Specs add a mandatory pre-registration update gate. runnerd fetches GitHub's repository- or Organization-scoped official application descriptors before it requests the registration token, validates the GitHub-owned URL, filename, architecture, version, and SHA-256, and passes no API credential into the Sandbox. A stale writable Runner requires `curl`, `tar`, `sha256sum`, `mktemp`, and `timeout`; the complete retrying download has a hard five-minute deadline and 512 MiB bound, verifies the archive and candidate version, and replaces the work directory before `config.sh`. The swap keeps the candidate and previous-directory backup under one random sibling update root until cutover completes; a catchable HUP, INT, or TERM after the old directory moves restores it, and a failed restoration preserves and reports the backup path. An installed Runner that matches or is newer than the target skips those tool and network requirements and is never downgraded. Lookup or Sandbox preflight failure prevents registration; the server-side lookup is recorded as `github_runner_downloads`, while an archive-version mismatch reports both the observed and target versions. Because the template is immutable, the same download is expected on every new Sandbox until the user rebuilds it. Managed Specs do not use this gate and continue to rely on their tested pin. Production acceptance for a deliberately stale custom template requires the detail page to show `runner_version != effective_runner_version` for a Job that completed on the refreshed Runner.

`POST /runner_requests/{id}/network-diagnostics` is a separate admin-only action for a request that is still `running` with a persisted Sandbox ID and Runner PID. Its JSON body accepts only `github_api`, `ubuntu_archive`, or `llvm_apt` as `target`; the server never accepts a URL, host, command, header, or credential from the browser and does not follow redirects. The fixed Sandbox command records at most eight DNS addresses, the connected IP, DNS/TCP/TLS/first-byte/total timings, HTTP status, exit code, and at most 512 bytes of a stable probe-error category. Raw command stderr and provider details are discarded. The complete action is limited to 10 seconds, with a three-second connection timeout, eight-second transfer timeout, and 65,536-byte range/maximum download. Combined command stdout/stderr is independently limited to 65,536 bytes and exceeding that budget cancels the command. Only one probe can run per Request ID/Sandbox ID/PID attempt, and a completed attempt has a 30-second cooldown. The handler does not hold the Runner lifecycle lock during provider I/O, reloads the request after the provider call, and discards a result if status or attempt identity changed. A safe structured result or generic provider-failure result is appended to `control.log` under the `network_diagnostic` stage; it never changes request status, retry fields, termination ownership, cleanup, or the GitHub workflow timeout. Historical and terminal requests remain readable but cannot run a live probe.

New requests retain the original Workflow Job's terminal name, status, conclusion, runner name, and local observation time when a completion webhook, pre-start check, recovery, reconciler, or bounded post-runner-exit lookup observes that terminal result. The post-exit lookup runs only after Sandbox cleanup has completed or entered its retry state, so GitHub availability cannot delay that cleanup. A zero or mismatched Job ID and every nonterminal observation are ignored, so an assigned different Job cannot overwrite the original result and active requests keep live behavior. Repeated identical terminal observations preserve the first retained observation time, and requeueing a mismatched completed request clears the old snapshot together with its lifecycle and termination fields. Diagnostics return retained evidence with `lookup_status` and `source` set to `retained`; rows created before this schema increment continue to use a bounded live lookup marked `source: live`. Successful live lookups are cached for 30 seconds, and concurrent lookups for the same Job are coalesced. Filtering happens before the event limit so high-volume stdout/stderr output cannot displace lifecycle evidence. New lifecycle writes persist the first termination owner as `process_exit`, `workflow_job_webhook`, `manual_stop`, `recovery_cleanup`, or `failure_cleanup`; an observed process result also persists its nullable exit code. The diagnostic finding for an unobserved termination is emitted only when neither typed state nor stable `runner_exit`, `runner_hook`, or `runner_cleanup` control stages nor the historical message fallback provide evidence. Consequently, a successful GitHub-completed request with a completed cleanup event is not treated as an abnormal unobserved termination. GitHub lookup failure does not discard local evidence, and the endpoint never returns the saved Sandbox credential or raw webhook payload. Exact collection lookup uses `GET /runner_requests_lookup/{identifier}`, preferring an exact Runner Name before an internal Request ID when both namespaces contain the same value, and then navigates to the canonical ID route. `/diagnostics/runner-requests/{identifier}` remains as a compatibility alias with the same lookup precedence. Runner hook, exit, and cleanup control events use stable `runner_hook`, `runner_exit`, `sandbox_cleanup`, `github_cleanup`, and `runner_cleanup` stages while preserving legacy `control.log` reads. The Sandbox command result does not currently expose the actual exit signal, so runnerd does not infer one from an exit code or from a requested stop signal.

The Runner Requests page accepts the user-visible Runner Name or an internal request ID for exact lookup, resolves that lookup before navigation, then opens the canonical `/admin/runner_requests/{id}` resource page. Its table keeps every request field on one line and uses horizontal scrolling when the viewport is narrower than the full dataset; vertical scrolling remains on the page instead of a nested table scroller. The resource page aggregates request state, findings, a retained terminal GitHub Job result or historical live fallback with explicit source/freshness, and one chronological Run history surface. It displays persisted Started, Stopping, Completed, and Failed timestamps; Sandbox region, resolved template ID, template version, preinstalled Runner version, and effective Runner version; the localized termination source plus its stable identifier; and a process exit code only when one was observed. Historical or partially captured environment fields render as `-`. While a request has a live running attempt, the same page progressively exposes the fixed-target network diagnostic control and directly presents resolved addresses, the connected IP, phase timings, HTTP status, exit code, and stable error category. There is no free-form target input, and success or failure refreshes the existing timeline rather than creating a second diagnostic record surface or export format. Cleanup Duration is calculated only from `stopping_at` to the terminal timestamp selected by the current status; a request completed before Sandbox creation leaves `stopping_at` absent. Missing, invalid, inverted, or Go-zero timestamps render as unavailable, and `updated_at` is never used as a cleanup endpoint. Every persisted control/stdout/stderr event remains an individual timeline row, and its message and optional stage are shown directly in normal page flow without a nested output card or disclosure. The newest 200 mixed events load first, and `GET /runner_requests/{id}/events?before_id=<event-id>` retrieves older pages using an exclusive cursor. Active diagnostics refresh every five seconds through one or more exclusive `after_id` pages until caught up, while the older-history cursor remains independent, so newly persisted events are merged without gaps or loss of history already loaded by the administrator. The separate `/admin/diagnostics` page is reserved for runnerd runtime inspection; it contains the redacted summary and pprof discovery, and does not fetch or render the potentially large expvar snapshot until an administrator explicitly loads or refreshes it. Legacy `/admin/diagnostics?runner=...` links resolve the identifier and redirect to the canonical request resource.

The idle-runner sweeper uses the additional `idle_cleanup` termination source so a successful cleanup of a Runner that never accepted a Job is not mislabeled as failure cleanup.

Each accepted process-exit watcher is bound to the Sandbox ID and PID of its execution attempt. If a matching process result arrives after a webhook, administrator action, or recovery path already completed or scheduled cleanup, runnerd stores a previously missing exit code and appends a `runner_exit` event without changing the first termination source or repeating cleanup. A delayed callback from an older attempt is ignored when either identity value no longer matches.
