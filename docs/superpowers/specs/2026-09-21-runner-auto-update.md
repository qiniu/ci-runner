...
- If the custom template's writable Runner is stale, download under fixed time and 512-MiB bounds, verify checksum and extracted version, replace the writable directory, and only then run `config.sh`. Keep the previous directory until cutover completes and restore it after a catchable HUP, INT, TERM, or failed candidate move; preserve and report the backup if restoration fails. Fail closed before registration. A matching version bypasses the downloader and its tool requirements.
