...
Per-request state, findings, GitHub Job correlation, and lifecycle/output events belong to the canonical `/admin/runner_requests/{id}` resource page rather than the runtime diagnostics payload. The page displays the persisted Started, Stopping, Completed, and Failed timestamps and derives Cleanup Duration only from `stopping_at` and the status-selected terminal timestamp. A request that reaches completion without entering cleanup leaves `stopping_at` absent. Runner exit, completed Hook, Sandbox cleanup, GitHub Runner cleanup, and overall cleanup control events use stable stage identifiers in the mixed timeline while legacy `control.log` reads remain compatible. The request record additively persists the first termination owner and a nullable process exit code; retrying or requeueing a fresh attempt clears that evidence. Diagnostics prefer these typed fields and stable stages, then retain the old control-message checks only for historical rows. The provider command result has no actual exit-signal field, so the service does not infer a signal from exit codes or stop requests. Runnerd retains a terminal result only when the observed Job ID equals the request's original `workflow_job_id`; the snapshot keeps name, status, conclusion, runner name, and local observation time independently from the Runner lifecycle status. The detail API prefers that retained evidence and labels its source/freshness; historical rows without a snapshot keep the bounded, cached, coalesced live GitHub lookup fallback.

Accepted process watchers are scoped to their Sandbox ID and PID. A matching exit result may add a missing exit code after another path has taken cleanup ownership, but it cannot repeat cleanup or overwrite the owner; stale attempt callbacks are ignored.
