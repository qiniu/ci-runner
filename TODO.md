...
- After the `feat/runner-environment-snapshot` increment is reviewed and deployed, run a real GitHub Actions Job and verify the persisted Sandbox region, resolved physical template ID, template version, and Runner version on the Admin request detail page, including historical-row compatibility. Then continue the [llgo Job timeout and runnerd diagnostics roadmap](docs/current-work-handoff.md) with separately bounded on-demand network diagnostics; actual exit signals remain unavailable from the Sandbox SDK, and the network root cause remains unverified.
