...
- Resume the [llgo Job timeout and runnerd diagnostics handoff](docs/current-work-handoff.md): lifecycle cleanup timestamps now preserve the real stopping transition and no longer place `stopping_at` after `completed_at`; environment snapshots and Job result retention remain unimplemented, and the network root cause remains unverified.
