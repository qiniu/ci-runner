...
- Complete [Issue #93](https://github.com/qiniu/ci-runner/issues/93) release promotion after the shared-source Actions Runner 2.337.0 change. The Ubuntu 24.04 development template version `20260917.3` reached `Status: ready` on 2026-09-17 (template `w54qc828mejc6x8sgacn`, build `56114e2c-b41d-4239-aa6b-134a8da39c31`), and all nine then-current Sandbox smoke checks passed against the configured endpoint. Remaining: build the four `*-large-80g` replacement physical templates while retaining referenced unsuffixed IDs; migrate any standard template whose runtime free-space smoke fails; run release smoke in `cn-yangzhou-1` and `us-south-1`; verify representative workflows and cleanup; switch configured large custom Runner Specs to the verified replacement IDs; then publish and check the regional catalogs. Total rootfs size can exceed the requested free space, so a larger total alone does not establish the original build request. The new runtime free-space smoke check has only offline fixture coverage; the single dev canary does not establish the full release gate.
