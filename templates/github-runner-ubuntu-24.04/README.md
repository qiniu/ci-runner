...
This stable 8-vCPU, 8192-MiB Linux x86_64 template sets a 20-GiB minimum root
disk size when created. It uses the pinned Canonical-published Ubuntu 24.04 OCI index in
...
Build the standard template with `task template-build-ubuntu-24-04`, using
`qshell.sandbox.toml`. Build the large variant with
`task template-build-ubuntu-24-04-large`, using
