...
- For custom-template Runner preflight updates, cover repository and Organization downloads endpoints, strict official URL/filename/version/architecture/SHA-256 validation, the ten-minute cloned cache, and concurrent lookup coalescing. Execute the rendered Sandbox script to prove a stale Runner is replaced before `config.sh`, a matching version skips download tools, checksum, unsupported-architecture, or missing-tool failures never register, and a catchable interruption after moving the old work directory restores it without leaving a backup. A candidate-version mismatch must report both the observed and target versions. Lifecycle tests must prove only custom Specs receive descriptors and that lookup failure occurs at `github_runner_downloads` before registration-token or Sandbox creation. Managed Specs must remain independent from the downloads endpoint.
